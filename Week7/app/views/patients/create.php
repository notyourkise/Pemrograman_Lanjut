<?php $title = 'Tambah Pasien - Week7'; require __DIR__ . '/../layout/header.php'; ?>
<div class="stack">
  <div class="card">
    <div class="breadcrumb"><a href="/MATERI-ASDOS/Week7/public/">Home</a><span class="sep">/</span><a href="?c=patients&a=index">Pasien</a><span class="sep">/</span><span>Tambah</span></div>
  ...
  <div class="card">
    <form class="grid cols-2" method="post">
      ...
      <div class="field">
        <label>Telepon</label>
        <input type="text" name="phone" placeholder="+62 xxx-xxxx-xxxx" value="<?= htmlspecialchars($old['phone'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
        <?php if (!empty($errors['phone'])): ?><div class="error"><?= htmlspecialchars($errors['phone'], ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?>
      </div>
      <div class="field" style="grid-column: 1 / -1;">
        <label>Alamat</label>
        <textarea name="address" rows="3" placeholder="Alamat lengkap pasien"><?= htmlspecialchars($old['address'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
        <?php if (!empty($errors['address'])): ?><div class="error"><?= htmlspecialchars($errors['address'], ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?>
      </div>
